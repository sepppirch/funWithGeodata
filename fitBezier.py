...
def fit_single_bezier_curve(points, max_control_dist=0.01, penalty_strength=2.0):
    ...
    result = minimize(bezier_fit_error, init, args=(points, max_control_dist, penalty_strength), method="Powell")
...
def plot_bezier_curve(P0, P1, P2, P3):
    ...
    plt.plot(curve[:, 0], curve[:, 1], "b-", label="Bézier curve", color=col)
...
def reduceToBeziers(data, plot=False):
    points = np.array([[e[0], e[1]] for e in  data])
    pointsC =  np.array([e[2] for e in  data])
    #print(len(points))
    newpoints = []
        ...
        if i < len(points)-2:
            ...
            if (int(iter)) > 1:
                for i in range(int(iter)):
                    ...
                    extraP.append([p[0], p[1], p[2]])
        #else:


    plotpoints = np.array([[e[0], e[1]] for e in  extraP])
    ...
    for i in range(len(s)):
        ...
        if i < len(s)-2:
            ...
            if i == len(s)-2:
                ...
                splitseg = []
                #print(d)
            elif  turn > 30 or d > 0.01: #count %13 == 0 or
                finalPoints.append(splitseg)
                turn = 0
                d = 0
                splitseg = []
    ...
    for s in finalPoints:
            ...
            #print(dist)   
            P0, P1, P2, P3 = fit_single_bezier_curve(newp, dist/3, 10)
...
thisbezier =json.dumps({"points":[{"coordinates":reduceToBeziers(data["coordinates"])}]})
    #print(Tin)
