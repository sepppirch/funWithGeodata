...
def trafficPaths(prefix, name, fclass):
                ...
                for p in trains["features"][start]["geometry"]["coordinates"]:
                    if p[0] >= 0 and p[0] < 1 and p[1] >= 0 and p[1] < 1:
                        ...
                        if trains["features"][start]["properties"]["bridge"] == "T" or trains["features"][start]["properties"]["tunnel"] == "T":
                            ...
                            c0 = hmap[int(trains["features"][start]["geometry"]["coordinates"][0][1]*32656)][int(trains["features"][start]["geometry"]["coordinates"][0][0]*32656)]*1.5625 - 51200
                            clast = hmap[int(trains["features"][start]["geometry"]["coordinates"][bseg][1]*32656)][int(trains["features"][start]["geometry"]["coordinates"][bseg][0]*32656)]*1.5625 - 51200
                            step = (clast - c0)/bseg
                            print(bseg)
                            c = c0 + step*count
                        else:
...
r3 = ["rail"]
data = {"roads1":trafficPaths('roadssmooth','0_-1', r1), "roads2":trafficPaths('roadssmooth','0_-1', r2), "rail":trafficPaths('rail','0_-1', r3)}
